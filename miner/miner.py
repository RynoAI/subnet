...
from ryno.metaclasses import ServiceRegistryMeta
...
class StreamMiner:
    ...
    def init_axon(self):
        ...
        if self.axon is not None:
            ...
            self.axon = RynoAxon(wallet=self.wallet, port=self.config.axon.port)
    ...
    def check_hotkey_validation(self):
        if self.wallet.hotkey.ss58_address not in self.metagraph.hotkeys:
            bt.logging.error(
                ...
                f"\nRun btcli recycle_register --netuid 224 and try again. "
            )
            sys.exit()
        else:
