...
import wandb
import ryno
from ryno import utils, dendrite
...
def parse_arguments():
    ...
    parser.add_argument("--netuid", type=int, default=224)
...
def init_wandb(config):
    ...
    run_name = f"validator-{config.wallet.hotkey.ss58_address}-{ryno.__version__}"
    config.run_name = run_name
    config.version = ryno.__version__
    config.type = "validator"

    run = wandb.init(
        name=run_name,
        project=ryno.PROJECT_NAME,
        entity="ryno",
    ...
    bt.logging.success(f"Started wandb run for project '{ryno.PROJECT_NAME}'")


async def close_all_connections():
    tasks = []
    for key, connection in dendrite.RynoDendrite.miner_to_session.items():
...
def main():
    ...
    config.dendrite = dendrite.RynoDendrite(wallet=config.wallet)
    ...
    try:
        ...
        bt.logging.info(
            f"closing all sessins. total connections is {len(dendrite.RynoDendrite.miner_to_session.keys())}")
